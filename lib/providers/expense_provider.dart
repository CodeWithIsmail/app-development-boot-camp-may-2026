...
import 'package:mexpense/helper/data.dart';
...
class ExpenseProvider extends ChangeNotifier {
  ...
  Map<String, double> dailyTotals(DateTime endDate, {int days = 15}) {
    ...
    for (final expense in _expenses.where((item) => item.title == 'Expense')) {
      final DateTime expenseDay = DateTime(
        expense.dateTime.year,
        expense.dateTime.month,
        expense.dateTime.day,
      );
      if (expenseDay.isBefore(startDate) || expenseDay.isAfter(normalizedEnd)) {
        continue;
      }

      final String day = dayFormat.format(expenseDay);
